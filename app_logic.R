...
mle_logic <- function(input, output, session) {
  ...
  output$likPlotMu <- renderPlot({
    ...
    mu_range <- seq(-3, 3, length.out = 100)
  ...
  output$likPlotSigma <- renderPlot({
    ...
    sig_range <- seq(0.1, 4, length.out = 100)
...
# pearson_logic <- function(input, output, session) {
#   
#   # --- Parameter constraints ---
#   
#   # enforce feasible skew/kurtosis combination
#   observe({
#     req(input$p_skew, input$p_kurt)
#     
#     min_kurt <- (input$p_skew^2) + 1.1
#     
#     if (input$p_kurt < min_kurt) {
#       updateSliderInput(
#         session, "p_kurt",
#         value = min_kurt + 0.1,
#         min = min_kurt
#       )
#     } else {
#       updateSliderInput(session, "p_kurt", min = min_kurt)
#     }
#   })
#   
#   # --- Data generation ---
#   
#   pearson_data <- reactive({
#     
#     req(input$p_n, input$p_mu, input$p_var, input$p_skew, input$p_kurt)
#     
#     # dependency on resample button
#     input$resample_p 
#     
#     moments <- c(
#       mean = input$p_mu,
...
#     
#     dat <- tryCatch({
#       rpearson(input$p_n, moments = moments)
#     }, error = function(e) {
#       # fallback if parameters temporarily clash
#       rnorm(input$p_n, mean = input$p_mu, sd = sqrt(input$p_var))
#     })
#     
#     dat
#   })
...
#     )
#     
#     x_seq <- seq(-8, 8, length.out = 200)
#     
#     # theoretical PDF
#     y_vals <- tryCatch({
#       dpearson(x_seq, moments = moments)
#     }, error = function(e) {
#       dnorm(x_seq, input$p_mu, sqrt(input$p_var))
#     })
#     
#     # mean / median / mode
#     th_mean <- input$p_mu
#     th_median <- tryCatch(
#       qpearson(0.5, moments = moments),
#       error = function(e) input$p_mu
#     )
#     th_mode <- x_seq[which.max(y_vals)]
#     
#     par(mar = c(4, 4, 2, 1))
#     
#     plot(
...
#       xlab = "x", ylab = "Density",
#       xlim = c(-7, 7), ylim = c(0, 1.2),
#       main = "Theoretical PDF"
#     )
#     
#     abline(v = th_mean, col = "red", lwd = 2, lty = 2)
#     abline(v = th_median, col = "blue", lwd = 2, lty = 3)
#     abline(v = th_mode, col = "darkgreen", lwd = 2, lty = 4)
#     
#     legend(
#       "topright",
